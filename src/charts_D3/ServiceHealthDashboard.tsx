...
const ServiceHealthDashboard: React.FC<ServiceHealthDashboardProps> = ({
  ...
  // Use default latencyHistory if not provided
  const latencyHistory = data.latencyHistory || defaultData.latencyHistory;
  ...
  const maxLatency = Math.max(...latencyHistory.map(d => d.p99));
  const xScale = (i: number) => (i / (latencyHistory.length - 1)) * innerChartWidth;
          ...
          <div style={{ 
            ...
            <svg width={chartWidth} height={chartHeight} style={{ display: 'block' }}>
              <g transform={`translate(${chartMargin.left}, ${chartMargin.top})`}>
                ...
                {/* X-axis labels */}
                {[0, 6, 12, 18, 23].filter(i => i < latencyHistory.length).map((i) => (
                  <text
                    key={i}
                    x={xScale(i)}
