...
// ============================================
// ENTERPRISE GOVERNANCE COLOR TOKENS
// 39 New Charts for Regulated AI/ML
// ============================================

// Risk Tier Colors (grayscale semantic)
...
export const vendorRisk = {
  tier1: '#333',  // Critical vendors
  tier2: '#555',  // Important vendors
  tier3: '#777',  // Standard vendors
  tier4: '#999',  // Low-risk vendors
};
...
// Enterprise Chart Palettes
export const enterprisePalette = {
  // 8-step grayscale for sequential data
  sequential: ['#1a1a1a', '#333', '#444', '#555', '#666', '#777', '#888', '#999'],
  
  // 5-step for categorical (enough distinction)
  categorical: ['#222', '#444', '#666', '#888', '#aaa'],
  
  // 3-step for simple status
  status: ['#333', '#666', '#999'],
...
// Heatmap Intensity (11 steps for fine gradation)
...
export const gaugeThresholds = {
  excellent: { min: 90, color: '#333' },
  good: { min: 70, color: '#555' },
  acceptable: { min: 50, color: '#777' },
  warning: { min: 30, color: '#888' },
  critical: { min: 0, color: '#aaa' },
};

// Helper: Get risk tier color
export function getRiskColor(tier: 'critical' | 'high' | 'medium' | 'low' | 'minimal'): string {
  return riskTiers[tier];
}

// Helper: Get compliance color
export function getComplianceColor(status: keyof typeof complianceStatus): string {
  return complianceStatus[status];
}

// Helper: Get heatmap color by value (0-100)
export function getHeatmapColor(value: number): string {
  const index = Math.min(10, Math.max(0, Math.round(value / 10)));
  return heatmapIntensity[index];
}

// Helper: Get gauge color by percentage
export function getGaugeColor(percent: number): string {
  if (percent >= 90) return gaugeThresholds.excellent.color;
  if (percent >= 70) return gaugeThresholds.good.color;
  if (percent >= 50) return gaugeThresholds.acceptable.color;
  if (percent >= 30) return gaugeThresholds.warning.color;
  return gaugeThresholds.critical.color;
}
